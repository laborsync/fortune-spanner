...
module.exports = Adapter => class SpannerAdapter extends Adapter {
    ...
    connect() {
        ...
        if (!this.database) {
            this.spanner = Spanner(parameters);
            this.instance = this.spanner.instance(parameters.instanceId);
            this.database = this.instance.database(parameters.databaseId);
        }
    ...
    _computeTableIndexColumnSets(type, ids, options, columnSets) {
        ...
        for (const option in options) {
            ...
            for (const column in options[option]) {
                ...
                if (logicalOperators.indexOf(column) >= 0) {
                    columnSets = this._computeTableIndexColumnSets(type, null, options[option][column], columnSets)
                    continue;
                }

                columnSets[columnSet][column] = true;
    ...
    _computeTableIndexRanks(type, ids, options) {
        ...
        const columnSets = this._computeTableIndexColumnSets(type, ids, options);
        ...
        for (const typeIndex in typeIndexes) {
            ...
            columnSets.forEach((columnSet) => {
                ...
                const rank = typeIndexes[typeIndex].filter((column) => {
                    return columnSet[column];
                }).length;

                if (ranks[typeIndex] < rank) {
                    ranks[typeIndex] = rank;
                }
